...
void EEShashPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  xBeam = (G4UniformRand()*19-9.5)+(-18.5)*mm;
  yBeam = (G4UniformRand()*19-9.5)*mm;
  /*
  x = (G4UniformRand()*9-4.5)+(-23.0);
  y = (G4UniformRand()*9-4.5)+(-4.5);
  if (y+9 < x+27.5 - 0.25){
    x = y-18.0;
    y = x+18.5;
  }
  ...
  */
  ...
  /*
  if (x > 0){
    xBeam = (G4UniformRand()*2-1)+(-18.5)*mm;
    yBeam = (G4UniformRand()*2-1)*mm;
  }else{
    xBeam = (G4UniformRand()*2-1)+(-25.5)*mm;
    yBeam = (G4UniformRand()*2-1)+(-7.0)*mm;
  }*/
